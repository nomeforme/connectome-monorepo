...
# Signal Axon — Signal platform adapter
#
# Self-contained build — no shared files from the monorepo.
#
# Packages: signal-axon, axon-interfaces, axon-server, axon-binding,
...
RUN --mount=type=cache,id=pnpm-store-signal-axon,target=/root/.local/share/pnpm/store \
...
RUN --mount=type=cache,id=turbo-cache-signal-axon,target=/workspace/node_modules/.cache/turbo \
    pnpm turbo run build --filter=@connectome/signal-axon
