/**
 * WhatsAppCommandEffector - Handles ! commands
 *
 ...
 * - !mt N - Set max output tokens per bot
 * - !help - Show help
 */
 ...
 * WhatsAppCommandEffector - Handles ! commands
 */
export class WhatsAppCommandEffector {
  private botName: string;
  /** Tracks the last-set maxOutputTokens override */
   ...
   * @param emitEvent - Optional callback to emit events to Connectome
    ...
    switch (command) {
      ...
      case '!mt': {
        ...
        if (emitEvent) {
          ...
          }).catch((e: any) => console.error(`[WhatsAppCommandEffector:${this.botName}] Failed to emit config event:`, e.message));
        }
  ...
  private getHelpText(config: RuntimeConfig): string {
    return `WhatsApp AXON Commands (use without argument to show current value):
